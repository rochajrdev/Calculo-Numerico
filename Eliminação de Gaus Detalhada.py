def eliminacao_de_gauss_detalhada(A, b):
    """
    Resolve um sistema linear Ax = b usando o método de Eliminação de Gauss,
    mostrando todos os passos, pivôs e novas matrizes.
...
# Exemplo de uso:
A = [
    [2, 3, -1],   # Coeficientes da primeira equação: 2x0 - x1 + x2 = 5
    [4, -4, -3],    # Coeficientes da segunda equação: x0 + 2x2 = 6
    [6, -3, 1]    # Coeficientes da terceira equação: 3x0 + 2x1 - x2 = 4
]
b = [5, 3, 3]  # Termos independentes correspondentes às equações acima

# Chamando a função para resolver o sistema e imprimir os passos
solucao = eliminacao_de_gauss_detalhada(A, b)